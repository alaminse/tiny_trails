...
@section('css')
<style>
    .broadcast-card { border-left: 4px solid #ffc107; transition: box-shadow 0.2s; }
    .broadcast-card:hover { box-shadow: 0 4px 14px rgba(0,0,0,.1) !important; }
    .broadcast-card.filled  { border-left-color: #198754; }
    .broadcast-card.expired { border-left-color: #dc3545; opacity: .75; }
    .timer { font-family: monospace; font-weight: 700; font-size: 14px; }
</style>
@endsection
                    ...
                    <div class="card card-primary card-outline mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <button class="btn btn-primary btn-sm"
                                    data-bs-toggle="modal" data-bs-target="#modalBroadcast">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div class="tab-pane fade show active" id="tabOpen">
                                                    ...
                                                    <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
                                                        <div class="flex-grow-1">
                                                            ...
                                                            <div class="d-flex flex-wrap gap-3 text-muted small">
                                                                <span><i class="bi bi-calendar3"></i> {{ \Carbon\Carbon::parse($b->ride->date)->format('D d M') }}</span>
                                                                <span><i class="bi bi-clock"></i> {{ \Carbon\Carbon::parse($b->ride->pickup)->format('g:i A') }}</span>
                                                                <span><i class="bi bi-geo-alt"></i> {{ $b->broadcast_area }}</span>
                                                            </div>
                                                        </div>
                                                        <div class="text-center">
                                                            <div class="timer text-warning" data-expires="{{ $b->expires_at }}">
                                                                ⏱ calculating…
                                                            </div>
                                                        ...
                                                        <div class="d-flex flex-column align-items-end gap-2">
                                                            ...
                                                            <form action="{{ route('admin.shift.broadcast.cancel', $b->id) }}" method="POST">
                                                                @csrf @method('PATCH')
                                            ...
                                            @empty
                                                ...
                                                <div class="card broadcast-card shadow-sm mb-3">
                                                    <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
                                                        <div class="flex-grow-1">
                                                            <div class="fw-bold mb-1">📍 Ghan NT → Sunshine Primary School</div>
                                                            <div class="d-flex flex-wrap gap-3 text-muted small">
                                                ...
                                                <div class="card broadcast-card shadow-sm mb-3">
                                                    <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
                                                        <div class="flex-grow-1">
                                                            <div class="fw-bold mb-1">📍 Clayton VIC → St. Joseph's College</div>
                                                            <div class="d-flex flex-wrap gap-3 text-muted small">
                                                                <span>📅 Mon 24 Feb</span>
                                                                <span>⏰ 7:30 AM</span>
                                                                <span>👦 2 kids</span>
                                        ...
                                        <div class="tab-pane fade" id="tabFilled">
                                            <table class="table table-hover mb-0">
                                                <thead class="table-light">
                                                    <tr>
                                                        <th>Route</th><th>Date</th><th>Driver</th><th>Accepted At</th><th>Status</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @forelse($filledBroadcasts ?? [] as $b)
                                                        <tr>
                                                            <td>{{ $b->ride->pickupLocation->address }} → {{ $b->ride->dropoffLocation->address }}</td>
                                                            <td><code>{{ \Carbon\Carbon::parse($b->ride->date)->format('D d M') }}</code></td>
                                                            <td>{{ $b->acceptance->driver->user->first_name ?? '—' }}</td>
                                                            <td><code>{{ \Carbon\Carbon::parse($b->acceptance->accepted_at)->format('g:i A') }}</code></td>
                                                            <td><span class="badge bg-success">Filled</span></td>
                                                        </tr>
                                                    @empty
                                                        <tr><td>Ghan NT → Sunshine Primary</td><td><code>Mon 24 Feb</code></td><td>James O.</td><td><code>06:45 AM</code></td><td><span class="badge bg-success">Filled</span></td></tr>
                                                        <tr><td>Clayton → St. Joseph's</td><td><code>Mon 24 Feb</code></td><td>Sarah K.</td><td><code>06:52 AM</code></td><td><span class="badge bg-success">Filled</span></td></tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
                                        </div>
                                        ...
                                        <div class="tab-pane fade" id="tabExpired">
                                            ...
                                            <table class="table table-hover mb-0">
                                                <thead class="table-light">
                                                    <tr><th>Route</th><th>Date</th><th>Expired At</th><th>Action</th></tr>
                                                </thead>
                                                <tbody>
                                                    @forelse($expiredBroadcasts ?? [] as $b)
                                                        <tr>
                                                            ...
                                                            <td><code>{{ \Carbon\Carbon::parse($b->ride->date)->format('D d M') }}</code></td>
                                                            <td><code>{{ \Carbon\Carbon::parse($b->expires_at)->format('g:i A') }}</code></td>
                                                            <td>
                                                                <form action="{{ route('admin.shift.broadcast.store') }}" method="POST" class="d-inline">
                                                                    @csrf
                                                                    <input type="hidden" name="ride_id" value="{{ $b->ride_id }}">
                                                                    <button class="btn btn-sm btn-primary">Re-broadcast</button>
                                                                </form>
                                                            </td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            ...
                                                            <td><button class="btn btn-sm btn-primary">Re-broadcast</button></td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
                            ...
                            <select class="form-select" name="ride_id" required>
                                ...
                                @foreach($unassignedRides ?? [] as $ride)
                                    <option value="{{ $ride->id }}">
                                        {{ $ride->date }} · {{ $ride->pickupLocation->address }} → {{ $ride->dropoffLocation->address }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        document.querySelectorAll('.timer[data-expires]').forEach(el => {
            function tick() {
                ...
                if (mins < 20) { el.classList.remove('text-warning'); el.classList.add('text-danger'); }
            }
            tick();

...
<div class="mt-3 d-flex gap-2 align-items-center">
    @if ($timesheet->status !== 'approved')
        @can('approve-timesheets')
            <button class="btn btn-success btn-sm btn-action-modal" data-id="{{ $timesheet->id }}" data-action="approved">
                ✅ Approve
            </button>
        @endcan
    @endif
    @if ($timesheet->status !== 'rejected')
        @can('reject-timesheets')
            <button class="btn btn-danger btn-sm btn-action-modal" data-id="{{ $timesheet->id }}" data-action="rejected">
                ❌ Reject
            </button>
        @endcan
    @endif

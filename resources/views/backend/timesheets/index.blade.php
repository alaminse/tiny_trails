        ...
        <div class="app-content">
            <div class="container-fluid">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    @can('list-timesheets')
                        <a href="{{ route('admin.timesheets.export', request()->query()) }}" class="btn btn-sm btn-outline-success">
                            ...
                            <table class="table ts-table table-hover mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Attendance</th>
                                        <th>Status</th>
                                        @canany(['approve-timesheets', 'reject-timesheets', 'view-timesheets'])
                                            <th>Actions</th>
                                        @endcanany
                                    </tr>
                                </thead>
                                        ...
                                        <tr data-ts-id="{{ $ts->id }}">
                                            ...
                                            <td>
                                                <div class="fw-semibold">
                                                    {{ \Carbon\Carbon::parse($ts->date)->format('d M Y') }}</div>
                                                <div class="text-muted small">
                                                    {{ \Carbon\Carbon::parse($ts->date)->format('l') }}</div>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                @if (($ts->total_rides ?? 0) > 0)
                                                    ...
                                                    <div class="attendance-bar">
                                                        <div class="attendance-bar-fill" style="width:{{ $pct }}%">
                                                        </div>
                                                    </div>
                                                    <div class="text-muted" style="font-size:.7rem">{{ $pct }}% done
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @php
                                                    ...
                                                    $attClass =
                                                        $attStatus === 'present' ? 'badge-present' : 'badge-absent';
                                            ...
                                            <td>
                                                <div class="d-flex gap-1 flex-wrap">
                                                    @can('approve-timesheets')
                                                        <button class="btn-approve btn-action" data-id="{{ $ts->id }}"
                                                    ...
                                                    @can('reject-timesheets')
                                                        <button class="btn-reject btn-action" data-id="{{ $ts->id }}"
                                                    ...
                                                    @can('view-timesheets')
                                                        <button class="btn btn-sm btn-outline-secondary btn-view"
                                                            data-id="{{ $ts->id }}" data-bs-toggle="modal"
        ...
        $(function() {
            ...
            $(document).on('click', '.btn-action', function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            const badgeClass = action === 'approved' ? 'badge-approved' :
                                'badge-rejected';
            ...
            $(document).on('click', '.btn-view', function() {
                ...
                $.get(`{{ url('timesheets') }}/${id}/detail`, function(html) {
                    ...
                    $('#tsDetailBody').html('<div class="alert alert-danger">Failed to load: ' + xhr
                        .responseText + '</div>');
                });
            });

                ...
                <div class="col-xl-8">
                    ...
                    @if ($existingShifts->isNotEmpty())
                        <div class="alert alert-warning d-flex gap-3 align-items-start">
                            ...
                            <div>
                                <div class="fw-bold mb-1">Shifts already exist for
                                    {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</div>
                                <div class="small">
                                    Submitting will <strong class="text-danger">delete all existing shifts and ride
                                        assignments</strong> for this date, then recreate fresh.
                                </div>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-5">
                                        ...
                                        <input type="date" name="date" id="dateInput" class="form-control"
                                            value="{{ old('date', $date) }}" min="{{ today()->toDateString() }}" required>
                                        <div class="form-text">
                                            Shifts are <strong>global per date</strong> — drivers are assigned per shift
                                            separately.
                                        </div>
                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    <div class="d-flex rounded overflow-hidden"
                                        style="height:30px;font-size:.72rem;font-weight:700;">
                                ...
                                @foreach ($shiftDefs as $num => $def)
                                    <input type="hidden" name="shifts[{{ $num }}][shift_number]"
                                        value="{{ $num }}">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="number" class="form-control"
                                                    ...
                                                    value="{{ old("shifts.{$num}.instant_seats", 0) }}" min="0"
                                                    max="10">
                        ...
                        @if ($unshiftedRides->isNotEmpty())
                            <div class="card shadow-sm border-0 mb-4">
                                <div class="card-header fw-bold" style="background:#fff3cd">
                                    ...
                                    <span
                                        class="badge bg-warning text-dark ms-1">{{ $unshiftedRides->flatten()->count() }}</span>
                                    <span class="text-muted fw-normal small ms-2">— Assign from the shift detail page after
                                        creating</span>
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="text-center">
                                @if($existingShifts->isNotEmpty())
                                    ...
                                    <button type="submit" class="btn btn-danger px-5 fw-semibold"
                                        ...
                                        ⚠ Delete & Recreate Shifts
                                    </button>
                                    @else
                                    <div class="alert alert-danger py-2 px-3 mb-0 small">
                                        <i class="bi bi-lock-fill"></i>
                                        You don't have permission to delete and recreate existing shifts.

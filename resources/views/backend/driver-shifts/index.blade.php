                                ...
                                <div class="card shift-card shift-{{ $shift->shift_number }} h-100 shadow-sm border-0">
                                    ...
                                    <div class="card-header d-flex justify-content-between align-items-center py-2 px-3">
                                        <span class="fw-bold">
                                            ...
                                            <span
                                                class="fw-normal small ms-1">{{ $shift->start_time }}–{{ $shift->end_time }}</span>
                                        </span>
                                        <div class="d-flex gap-1 align-items-center">
                                            <span
                                                class="badge bg-{{ $shift->status === 'confirmed' ? 'success' : ($shift->status === 'active' ? 'info' : 'secondary') }}">
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="mb-3">
                                            <div class="d-flex justify-content-between align-items-center mb-1">
                                                ...
                                                <button type="button" class="btn btn-outline-primary py-0 px-2"
                                                    style="font-size:.72rem" onclick="toggleAddDriver({{ $shift->id }})">+
                                                    Add</button>
                                                @endcan
                                            </div>
                                            @forelse($shift->drivers as $driver)
                                                <span class="driver-chip">
                                                    ...
                                                    @if ($shift->status === 'draft')
                                                        ...
                                                        <form
                                                            action="{{ route('admin.driver.shifts.removeDriver', [$shift->id, $driver->id]) }}"
                                                            method="POST" style="display:inline"
                                                            onsubmit="return confirm('Remove driver?')">
                                                            @csrf @method('DELETE')
                                                            <button
                                                                style="cursor:pointer;color:#dc3545;background:none;border:none;padding:0;font-size:.82rem">✕</button>
                                                        </form>
                                                        @endcan
                                                    @endif
                                                </span>
                                        ...
                                        <div class="d-flex gap-2 mb-3 flex-wrap">
                                            ...
                                            <span class="badge bg-light text-dark border">{{ $shift->shiftRides->count() }}
                                                assigned</span>
                                        </div>
                                            ...
                                            <div class="ride-row d-flex justify-content-between align-items-center">
                                                <div>
                                                    <span class="badge bg-secondary me-1"
                                                        style="font-size:.68rem">#{{ $sr->seat_number }}</span>
                                                ...
                                                @if (in_array($shift->status, ['draft', 'confirmed']))
                                                    ...
                                                    <form
                                                        ...
                                                        @csrf @method('DELETE')
                                                        <button class="btn py-0 px-1 btn-outline-danger"

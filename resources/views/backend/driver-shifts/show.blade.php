...
<style>
    /* ── Seat Grid ── */
    .seat-grid { display:flex; gap:8px; flex-wrap:wrap; margin:8px 0 20px; }
    .seat-box {
        width:42px; height:42px; border-radius:8px;
        display:flex; align-items:center; justify-content:center;
        font-size:12px; font-weight:700; border:2px solid #dee2e6;
        transition:transform .15s, box-shadow .15s; cursor:default; position:relative;
    }
    .seat-box.clickable       { cursor:pointer; }
    .seat-box.clickable:hover { transform:scale(1.18); box-shadow:0 3px 10px rgba(0,0,0,.2); }
    .seat-box.booked          { background:#0d6efd; color:#fff; border-color:#0a58ca; }
    .seat-box.booked.clickable:hover { background:#dc3545; border-color:#b02a37; }
    .seat-box.completed       { background:#adb5bd; color:#fff; border-color:#6c757d; }
    .seat-box.empty           { background:#f0f4ff; color:#6c757d; border-color:#c5d0e6; }
    .seat-box.empty.clickable:hover { background:#dbeafe; border-color:#0d6efd; color:#0d6efd; }
    .seat-box.active-seat     { outline:3px solid #0d6efd; outline-offset:2px; }
    .seat-box.pulse           { animation:pulse .4s ease; }
    @keyframes pulse { 0%{transform:scale(1)} 50%{transform:scale(1.3)} 100%{transform:scale(1)} }
    @keyframes shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-6px)} 75%{transform:translateX(6px)} }

    /* ── Driver section divider ── */
    .driver-seat-section { margin-bottom:14px; }
    .driver-seat-label {
    ...
    /* ── Ride Cards ── */
    .ride-card {
        border:2px solid #dee2e6; border-radius:10px; padding:11px 13px;
    ...
    .ride-card .kid-name  { font-weight:700; font-size:.9rem; }
    .ride-card .ride-meta { font-size:.74rem; color:#6c757d; margin-top:2px; }

    /* ── Route Header ── */
    .route-header {
        background:#1a1d24; color:#e2e8f0; padding:6px 13px;
        border-radius:8px; font-size:13px; font-weight:600; margin:12px 0 7px;
    }
    .time-badge {
    ...
    /* ── Left panel sticky ── */
    #assignPanel { position:sticky; top:76px; }

    /* ── Assigned list ── */
    .assigned-item {
        font-size:.8rem; padding:5px 0; border-bottom:1px solid #f0f0f0;
        ...
        <div id="assignPanel">
            ...
            <div class="card shadow-sm border-0 mb-3">
                ...
                <div class="card-header fw-bold"
                    ...
                    <span class="badge ms-2 bg-{{ $shift->status==='confirmed'?'success':($shift->status==='active'?'info':'light text-dark') }}">
                ...
                <div class="card-body pb-2">
                        ...
                        @forelse($shift->drivers as $d)
                            <span class="driver-chip">
                                ...
                                <span class="text-muted" style="font-size:.68rem">
                                    {{ $d->vehicleType?->name ?? '' }}
                                    ({{ $driverSeatData[$d->id]['free'] }}/{{ $driverSeatData[$d->id]['max'] }} free)
                                </span>
                                @if(in_array($shift->status, ['draft','confirmed']))
                                <form action="{{ route('admin.driver.shifts.removeDriver', [$shift->id, $d->id]) }}"
                                      method="POST" style="display:inline"
                                      onsubmit="return confirm('Remove driver?')">
                                    @csrf @method('DELETE')
                                    <button style="background:none;border:none;color:#dc3545;cursor:pointer;padding:0;font-size:.8rem">✕</button>
                                </form>
                                @endif
                            </span>
                        @empty
                    ...
                    <div class="driver-seat-section">
                        <div class="driver-seat-label">
                            ...
                            <span class="badge bg-{{ $sd['free']>0?'success':'danger' }} ms-1" style="font-size:.65rem">
                                {{ $sd['free'] }}/{{ $sd['max'] }} free
                            </span>
                        </div>
                        <div class="seat-grid">
                            @for($s = 1; $s <= $sd['max']; $s++)
                                @php
                                    ...
                                    $isDone  = $sr && in_array($sr->ride?->status, ['completed','cancelled','canceled']);
                                    $isBooked = $sr && !$isDone;
                                    $kidName  = $sr?->ride?->rideAssign?->subscription?->kid?->first_name ?? '';
                                @endphp

                                @if($isBooked)
                                    @php
                                        $pickupFmt  = \Carbon\Carbon::parse($sr->ride?->pickup)->format('H:i');
                                        $dropoffFmt = \Carbon\Carbon::parse($sr->ride?->drop_off)->format('H:i');
                                    @endphp
                    ...
                    @empty
                        <div class="text-muted small fst-italic mb-2">Add drivers above to see seat grid.</div>
                    @endforelse

                    {{-- Active seat info banner --}}
                    <div id="seatAssignBox" class="alert alert-primary py-2 px-3 small d-none
                                                    d-flex justify-content-between align-items-center">
                        <span>
                            Assigning to <strong>Seat <span id="activeSeatLabel">—</span></strong>
                            for <strong><span id="activeDriverLabel">—</span></strong>
                        </span>
                        <button type="button" class="btn-close btn-close-sm" onclick="cancelSeatSelect()"></button>
                    </div>
                    @endif
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body p-2">
                    ...
                    <div class="assigned-item px-1">
                        <div>
                            <span class="badge bg-{{ $sr->type==='completed'?'secondary':'primary' }} me-1"
                            ...
                            @if($sr->ride->driver)
                            <span class="badge bg-light text-dark border ms-1" style="font-size:.63rem">
                                @php
                                    $rideDriver = $sr->ride->driver;
                                    $driverName = $rideDriver?->first_name          // if driver IS a User
                        ...
                        <form action="{{ route('admin.driver.shifts.removeRide', [$shift->id, $sr->ride_id]) }}"
                              method="POST" onsubmit="return confirm('Remove ride? Seat will be freed.')">
                            @csrf @method('DELETE')
                ...
                <div class="card-body">
                    ...
                    @else
                        ...
                        @php
                            ...
                            $pickupAddr = $pickupLoc?->city
                                ?? $pickupLoc?->street1
                                ?? $pickupLoc?->address
                                ?? '—';
                            $dropoffAddr = $dropoffLoc?->city
                                ?? $dropoffLoc?->street1
                                ?? $dropoffLoc?->address
                                ?? '—';
                        @endphp
                            ...
                            <div class="d-flex align-items-start gap-3">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start">
                                        ...
                                        <span class="badge bg-{{ $isMorning ? 'primary' : 'warning text-dark' }} ms-2">
...
function clearActiveSeat() {
    ...
    _activeSeat = null; _activeDriver = null; _activeDriverName = '';
}
...
function assignRideToSeat(rideId) {
    ...
    .then(data => {
        if (data.success) {
            showToast('✅ Seat ' + _activeSeat + ' → ' + _activeDriverName + ' · ' + (data.seats_available) + ' seats left', 'success');
